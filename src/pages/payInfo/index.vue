...
<div id="payInfo">
    <div class="head">
        ...
        <div class="goods">
            <div class="goods-item" v-for="(item,index) in list" :key="index">
                <img-text-card :img="item.goods_head_list[0]" :price="item.price" :title="item.title" :oldPrice='item.o_price' :select_value="item.select_value"></img-text-card>
            </div>
        </div>
        
        <!-- 优惠券单元格 -->
        <van-coupon-cell class="coupon-cell" :coupons="coupons" :chosen-coupon="chosenCoupon" @click="showList = true" />
        <!-- 优惠券列表 -->
        <van-popup v-model="showList" round position="bottom" style="height: 90%; padding-top: 4px;">
            <van-coupon-list :coupons="coupons" :chosen-coupon="chosenCoupon" :disabled-coupons="disabledCoupons" @change="couponChange" :show-exchange-bar='false' />
        </van-popup>

        <div class="center-box">
            <van-cell-group>
                <van-field v-model="message" rows="1" :autosize="{ maxHeight: 200, minHeight: 50}" label="备注信息" type="textarea" placeholder="请输入备注信息" />
            </van-cell-group>
        </div>
    </div>
    <div class="f-tool">
    ...
    <van-popup position="bottom" style="height:90vh" v-model="show">
        <div id="select">
            <van-divider v-if="address.length==0" :style="{ color: 'black', borderColor: '#1989fa', padding: '0 16px' }">暂无数据</van-divider>
                ...
                <div v-for="(item,index) in address" :key="index" class="item" @click="change(item)">
                    ...
                    <div class="item-footer">
                        ...
                        <div class="footer-right">
                            <div class="operate">
                                <van-icon name="edit" @click.stop="$router.push(`/address/edit?id=${item.id}`)" style="font-size: 18px;" />编辑
                            </div>
                            <div class="operate">
